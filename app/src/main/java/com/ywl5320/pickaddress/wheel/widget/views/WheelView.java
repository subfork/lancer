 ...
 */
public class WheelView extends View {//这个滚筒显示的是 5 行数据

	/** Top and bottom shadows colors 渐变原理：上面2行数据顶端到底端的渐变，中间一行数据是独立的背景，下面2行数据背景渐变和上面2行数据是对称的
	 * 这个设计看上去就像从第1行顶端到第5行底端开始渐变，2头相同，中间过渡
	 * 
	 * 
	 * 这个阴影颜色控制着上2行，下2行数据的view背景颜色，可以自定义颜色，达到想要的效果
	 * 另外就是xml文件wheel_bg.xml控制着整个滚轮view的背景， wheel_val.xml控制着正中间的选中值的view的背景色
	 * 
	 * */
//	private static final int[] SHADOWS_COLORS = new int[] { 0xeeffffff, 0xeaffffff, 0x33ffffff };
	private static final int[] SHADOWS_COLORS = new int[] { 0xd3ffffff, 0xdaffffff, 0x33ffffff };
	...
	private void drawCenterRect(Canvas canvas) {
		...
		int offset = (int) (getItemHeight() / 2 * 1.08f);
		centerDrawable.setBounds(0, center - offset, getWidth(), center + offset);
		centerDrawable.draw(canvas);
		// TODO: 2016/9/12 绘制中间的那个选择线
		final Paint p = new Paint();
		p.setAntiAlias(true);
		p.setStrokeWidth(1.0f);
		p.setColor(Color.RED);
		canvas.drawLine(0,center-offset,getWidth(),center-offset,p);
		canvas.drawLine(0,center+offset,getWidth(),center+offset,p);
	}
